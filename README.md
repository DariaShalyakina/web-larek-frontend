...
**Конструктор:**
Конструктор компонента, который принимает контейнер HTML-элемента, в котором расположен продукт, и необязательные действия actions. 
Если определены actions, к кнопке добавляется обработчик событий click, который удаляет продукт из DOM и вызывает метод onClick из actions.

constructor(container: HTMLElement, actions?: IProductBasketActions)

**Сеттеры **
устанавливают соответствующие значения в элементы HTML.
+ title
+ index 
+ price 
...
- `IBasketView`: Описывает представление корзины в приложении.
- `IProductInBasket`: Представляет отдельный продукт в корзине.
